...
from FSGM import FGSM
from FGM import FGM
from PGD import PGD
from train.utils import read_examples, convert_examples_to_features
...
class TextRNNAtten(nn.Module):
    def __init__(self, config):
        ...
        self.tanh = nn.Tanh()
        self.w = nn.Parameter(torch.zeros(hidden_size * 1))
    ...
    def forward(self, x):
        ...
        alpha = F.softmax(torch.matmul(M, self.w), dim=1).unsqueeze(-1).unsqueeze(-1)
        return alpha
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, source_ids=None, source_mask=None, target_ids=None, target_mask=None, args=None):
        outputs = self.encoder(source_ids, attention_mask=source_mask, output_attentions=True, output_hidden_states=True)
        hidden_states = outputs.hidden_states
        # encoder_output = hidden_states[-1]
        cls_embeddings = hidden_states[1][:, 0, :].unsqueeze(1)
        encoder_output = hidden_states[1].unsqueeze(1)
        for i in range(2, 13):
            cls_embeddings = torch.cat((cls_embeddings, hidden_states[i][:, 0, :].unsqueeze(1)), dim=1)
            encoder_output = torch.cat((encoder_output, hidden_states[i].unsqueeze(1)), dim=1)

        # print('112----------encoder_output', encoder_output.shape)
        alpha = self.lstm(cls_embeddings)
        # print('114----------alpha', alpha.shape)
        encoder_output = encoder_output * alpha
        encoder_output = torch.sum(encoder_output, 1)
        # print('117----------encoder_output', encoder_output.shape)
...
class CodeBert_Seq2Seq():
    ...
    def train(self, train_filename, train_batch_size, num_train_epochs, learning_rate,
        ...
        fgsm = FGSM(self.model)
        fgm = FGM(self.model)
        pgd = PGD(self.model)
        K = 4

        # Prepare optimizer and schedule (linear warmup and decay)
        # codebert_params = list(map(id, self.model.encoder.parameters()))
        # other_params = filter(lambda p:id(p) not in codebert_params, self.model.parameters())
        ...
        # optimizer = torch.optim.AdamW([{'params':other_params}, {'params': self.model.encoder.parameters(), 'lr':5e-5}], lr=learning_rate,
        #                               eps=1e-8)
        ...
        for epoch in range(num_train_epochs):
            ...
            for batch in bar:
                ...
                # loss_adv = self.model(source_ids=source_ids, source_mask=source_mask, target_ids=target_ids,
                #                       target_mask=target_mask)
                # loss_adv = loss_adv[0]
